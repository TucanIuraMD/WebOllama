...
CASES = """
const cases = {
  good: { available: true, host: '192.168.80.22',
          ...
          gpus: [{ index: 0, name: 'Tesla V100 16GB', utilization: 74,
                   ...
                   temperature: 64 }], stale: false },
  multiGpu: { available: true, host: 'x', cpu: { utilization: 10, load: [1, 1, 1] },
  ...
  noGpu: { available: true, host: 'x', cpu: { utilization: 5, load: [1] }, gpus: [] },
  noTemp: { available: true, host: 'x', cpu: { utilization: 5, load: [1] },
            gpus: [{ index: 0, name: 'C', utilization: 9, memory_used: 1e9, memory_total: 2e9, temperature: null }] },
  down: { available: false, reason: 'remote host unreachable: ConnectError', cpu: null, gpus: [] },
  notConfigured: { available: false, configured: false, reason: 'not configured (PROCESSOR_URL)', cpu: null, gpus: [] },
  garbage: { available: true, host: 'x', cpu: { utilization: null, load: null }, gpus: [{ index: 0 }] },
};
let failures = 0;
for (const [name, data] of Object.entries(cases)) {
  ...
  const checks = {
    ...
    multiGpu: () => (html.match(/proc-card/g) || []).length === 3 &&
                   html.includes('#0') && html.includes('#1') && html.includes('99%') && html.includes('80\\u00b0C'),
    noGpu: () => html.includes('GPU information unavailable') && html.includes('5%'),
    noTemp: () => html.includes('NOT AVAILABLE'),
    down: () => html.includes('Processor information unavailable') && html.includes('unreachable'),
    notConfigured: () => html.includes('Processor information unavailable'),
